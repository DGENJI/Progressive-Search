...
A_star_result *search_hnsw(hnswlib::HierarchicalNSW<float> &hnsw, float *query, float *data, int N, int d, int kn, int kr, float thres, int ef)
{
    ...
    while (k < L)
    {
        ...
        if (k >= now_stop)
        {
            ...
            if (current_round == 1)
            {
                for (int i = 0; i < k / ef; i++)
                {
                    ...
                    for (int j = i + 1; j < k / ef; j++)
                    {
                        ...
                        for (int k = 0; k < d; k++)
                        {
                            // dist += data[knn[i].id * d + k] * data[knn[j].id * d + k];
                            dist += (data[knn[i].id * d + k] - data[knn[j].id * d + k]) * (data[knn[i].id * d + k] - data[knn[j].id * d + k]);
                        }
                        dist = 1 - sqrt(dist);
                        if (dist > thres)
                        {
            ...
            std::vector<int> degarr;
            for (int i = 0; i < k / ef; i++)
            {
                degarr.push_back(knn[i].size);
                std::sort(degarr.begin(), degarr.end(), std::greater<int>());
                if (degarr.size() > kr - 1)
                {
                    degarr.resize(kr - 1);
                }
...
int main()
{
    ...
    read_fvecs("../deep1M/deep1M_base.fvecs", d, N, data);
    cout << "data done" << endl;
    std::string index_path = "../deep1m_200_16_dist.bin";
    hnswlib::InnerProductSpace space(d);
    hnswlib::HierarchicalNSW<float> hnsw(&space, index_path, N);

    float *query_deep;
    int query_n;
    read_fvecs("../deep1M/deep1M_query.fvecs", d, query_n, query_deep);
    cout << "query done" << endl;
    for (int ef = 10; ef < 101; ef += 10)
    {

        char log_path[1024];
        sprintf(log_path, "../log/v1/deep/test/100_1000_%d_3.log", ef);
        ofstream logFile(log_path, std::ios::out);

        printf("Searching...\n");
        float simlist[3] = {0.6};

        for (int simindex = 0; simindex < 1; simindex++)
        {
            float sim = simlist[simindex];
            for (int kr = 5; kr < 25; kr += 5)
            {
                float period = 0;
                float value = 0;
                float bigk = 0;
                ...
                sprintf(single_log_path, "../log/v1/deep/deep1m_100_1000_%.2f_%d_%d.log", sim, kr, ef);
                ofstream single_logFile(single_log_path, std::ios::out);
                for (int qn = 0; qn < query_num; qn++)
                {
                    float *qq = query_deep + qn * d;
                    int t = clock();
                    A_star_result *rs = search_hnsw(hnsw, qq, data, N, d, kn, kr, 1 - sim, ef);
                    t = clock() - t;
                    period += t;
                    value += rs->score[kr];
                    bigk += rs->K;
                    ...
                    single_logFile << t << ' ' << rs->score[kr] << ' ' << rs->K << endl;
                    cout << sim << ' ' << kr << ' ' << qn << ' ' << t << endl;
                    delete rs;
                }
                single_logFile.close();
                logFile << sim << ' ' << kr << ' ' << period / query_num << ' ' << value / query_num << ' ' << bigk / query_num << endl;
            }
        }
        ...
        // t = clock();
        // int L_hnsw = search_hnsw(hnsw, hnsw_n, hnsw_d, query_deep, data, ids_hnsw, dists_hnsw, N, d, kn, kr, neiid, neidis, divs_hnsw, thres, 1000);
        // cout << L_hnsw << endl;
        // cout << "time for value=" << clock() - t << "ms" << endl;

        // float tol = 0;
        // for (int i = 0; i < L_hnsw; i++)
        // {
        //     tol += dists_hnsw[i];
        // }
        // cout << "tol=" << tol << endl;
